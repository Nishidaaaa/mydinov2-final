...
class _Split(Enum):
    ...
    def length(self) -> int:
        split_lengths = {
            # _Split.TRAIN: 359_984, #pathmnist
            # _Split.VAL: 40_016,
            # _Split.TEST: 0,
            # _Split.Test: 28_716,
            # _Split.TRAIN: 213_348, #CTS_100
            # _Split.VAL: 87_732,
            # _Split.TEST: 0,
            # _Split.Test: 137_330,
            _Split.TRAIN: 651_382, #CTS
            _Split.VAL: 263_611,
            _Split.TEST: 0,
            _Split.Test: 391_460,
            # _Split.TRAIN: 666_468, #Pilot
            # _Split.VAL: 723_096,
            # _Split.TEST: 0,
            # _Split.Test: 546_668,
        }
    ...
    def get_image_relpath(self,actual_index : int, class_id: Optional[str] = None) -> str:
        ...
        if self == _Split.TRAIN:
            ...
            basename = f"{self.value}_{actual_index:08d}"
            #basename = f"{self.value}_{actual_index:08d}"
...
class ImageNet(ExtendedVisionDataset):
    ...
    def get_image_data(self, index: int) -> bytes:
        ...
        #print('hi get_image_data')
        #entryファイルとclassidファイルからimage_dataの特性を読み込んでいる 
        image_relpath = self.split.get_image_relpath(actual_index, class_id)
        image_full_path = os.path.join(self.root, image_relpath)
        # 取得した画像ファイルをバイナリ形式で読み込む
        with open(image_full_path, mode="rb") as f:
            image_data = f.read()#ここで1枚読まれている
            #print(image_full_path)
            #print(image_data)
        return image_full_path, image_data
